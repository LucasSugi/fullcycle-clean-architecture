...
describe("Product unit tests", () => {
  ...
  it("should throw error when price is less than zero", () => {
    ...
    }).toThrow("product: Price must be greater or equal to zero");
  });

  it("should throw error when name and id is empty and price is less than zero", () => {
    expect(() => {
      const product = new Product("", "", -1);
    }).toThrow("product: Id is required,product: Name is required,product: Price must be greater or equal to zero");
  });
